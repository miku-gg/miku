...
async function main() {
    ...
    for (const line of envExampleLines) {
        ...
        if (trimmedLine.includes('=')) {
            ...
            if (currentValue === undefined && defaultValue.includes('<') && defaultValue.includes('>')) {
                const new_value = await prompt(`Enter value for ${keyName} (default: ${defaultValue}): `);
    ...
    const browserChatEnvFilePath = path.join(process.cwd(), 'apps', 'browser-chat', '.env');
...
main().then(() => console.log('Environment setup complete.')).catch(console.error);
