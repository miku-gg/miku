...
export async function* mikuCardUploader(
  ...
  if (rawMikuCard?.spec !== "chara_card_v2")
    throw "Invalid file type, only chara_card_v2 is supported";
  if (!rawMikuCard?.data?.extensions?.mikugg?.scenarios?.length)
    throw "Invalid card: extension.mikugg.scenarios not found or is empty";
  const errors = validateMikuCard(rawMikuCard);
  if (errors.length) throw errors.join("\n");
  const {
    card: _extractedMikuCard,
    images,
    audios,
  } = await extractMikuCardAssets(rawMikuCard);
  ...
  async function uploadAsset(
    ...
    const newAssetKey = await addAsset(
      asset,
      assets.get(asset) as string,
      callbacks.upload
    );
    return { find: asset, replace: newAssetKey };
  }

  const assetUploadPromises = Array.from(assets.keys()).map((asset) =>
    uploadAsset(asset)
  );
  ...
  hashes.forEach(({ find, replace }) => {
    _extractedMikuCard.data = replaceStringsInObject(
      _extractedMikuCard.data,
      find,
      replace
    ) as MikuCard["data"];
  });
  const newImages = new Map(
    [...images].map(([hash, base64URL]) => {
      const newHash = hashes.find((x) => x.find === hash)?.replace || hash;
      return [newHash, base64URL];
    })
  );

  const _extractedMikuCardHash =
    (await Hash.of(JSON.stringify(_extractedMikuCard))) + ".json";
  await callbacks.upload(
    BUCKET.BOTS,
    _extractedMikuCardHash,
    Buffer.from(JSON.stringify(_extractedMikuCard), "utf-8")
  );
  currentStep++;
  yield currentStep / totalSteps;
  ...
  if (callbacks.sharp) {
    ...
    const resizeImagePromises = Array.from(newImages.keys())
      .filter(
        (hash) =>
          hash.includes(".png") ||
          hash.includes(".jpg") ||
      ...
      .map((hash) =>
        resizeImage(
          ...
          (filename: string, data: Buffer) =>
            callbacks.upload(BUCKET.ASSETS, filename, data),
          hash,
