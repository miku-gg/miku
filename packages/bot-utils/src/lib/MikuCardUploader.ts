import { MikuCard, extractMikuCardAssets, replaceStringsInObject, validateMikuCard } from '..';
...
export async function* mikuCardUploader(
  ...
  hashes.forEach(({ find, replace }) => {
    _extractedMikuCard.data = replaceStringsInObject(_extractedMikuCard.data, find, replace) as MikuCard['data'];
  });
  ...
  if (callbacks.sharp) {
    ...
    const resizeImagePromises = Array.from(newImages.keys())
      .filter(
        (hash) => hash.includes('.png') || hash.includes('.jpg') || hash.includes('.jpeg') || hash.includes('.webp'),
      )
      .map((hash) =>
        resizeImage(
          sharp,
