...
export class RBPTStrategy extends ContextPromptBuildStrategy {
  buildContextPrompt(parts: ContextPromptParts): string {
    const { botSubject, persona, attributes, sampleChat, scenario, greeting } = parts;
  ...
  buildInitiatorPrompt(parts: ContextPromptParts): string {
    ...
    return greeting.replaceAll(`${botSubject}:`, `RPBT (${botSubject}):`).replaceAll('You:', 'Anon:');
  }
}
