...
import { TTSService, TTSServiceConfig, TTSServicePropTypes } from './TTSService';
...
export class AzureTTSService extends TTSService {
  ...
  constructor(config: AzureTTSServiceConfig) {
    super({
      ...config,
      apiEndpoint: config.apiEndpoint || 'https://westus.tts.speech.microsoft.com/cognitiveservices/v1',
    });
    this.apiKeyEndpoint = config.apiKeyEndpoint || 'https://eastus.api.cognitive.microsoft.com/sts/v1.0/issuetoken';
    this.tempKey = '';
    this.updateTempKey();
  }
  ...
  protected override async computeInput(input: InferProps<typeof TTSServicePropTypes>, tries = 0): Promise<string> {
    ...
    const tempKey = (input.apiKey) ? (await this.getTempKey(input.apiKey)) : this.tempKey;
    ...
    return axios<ArrayBuffer>({
      ...
      return "data:audio/x-wav;base64," + Buffer.from(response.data).toString('base64')
    }).catch(async (e) => {
      if (tries > 2) {
        console.error(e);
        return '';
