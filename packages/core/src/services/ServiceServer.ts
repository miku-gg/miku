...
export interface ServiceConfig {
  ...
  addRoute: (path: string, cb: (body: ServiceRequestBody) => Promise<{ status: number, response: any}>) => void;
}


function asRequired(propTypes: PropTypes.ValidationMap<any>): PropTypes.ValidationMap<any> {
...
export abstract class Service<Output = string> {
  ...
  constructor(config: ServiceConfig) {
    ...
    config.addRoute(`/${this.serviceId}/price`, async (body: ServiceRequestBody) => {
      try {
        const price = await this.calculatePrice(this.inputBase64ToProps(body.query.input));
        return {
          status: 200,
          response: {price}
        };
    ...
    config.addRoute(`/${this.serviceId}/query`, async (body: ServiceRequestBody) => {
      try {
  ...
  private isSignarureValid(requestBody: ServiceRequestBody): boolean {
    return verifyMessage(JSON.stringify(requestBody.query), requestBody.signature) === requestBody.query.address;
  }
  ...
  protected abstract computeInput(input: InferProps<typeof this.propTypes>): Promise<Output>;
  protected abstract calculatePrice(input: InferProps<typeof this.propTypes>): Promise<number>;
}