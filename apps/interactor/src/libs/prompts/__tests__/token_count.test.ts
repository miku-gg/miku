...
describe('Token Count', () => {
  test('should build a roleplay prompt', async () => {
    ...
    expect(strategy.countTokens('this is a random tests for counting tokens')).toBe(9);
    expect(strategy.countTokens('this is a random{{GEN sa max_tokens=150}} tests for counting tokens')).toBe(9 + 150);
    expect(
