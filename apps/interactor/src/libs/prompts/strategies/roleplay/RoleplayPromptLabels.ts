const labels: Record<string, Record<string, string>> = {
  ...
  es: {
    ...
    inner_thoughts_instruction: 'Pensamientos internos de % (punto de vista de Valerian, primera persona, solo pensamientos en bruto, sin formato). Esto es IMPORTANTE: para pensamientos internos, no debes describir acciones o movimientos, solo sentimientos, opiniones y reacciones mentales. Escribe tus pensamientos solo como texto, no uses corchetes, paréntesis o cualquier otro formato.',
  ...
  de: {
    ...
    inner_thoughts_instruction: 'Innere Gedanken von % (Valerians Sichtweise, erste Person, nur rohe Gedanken, ohne Formatierung). Das ist WICHTIG: für innere Gedanken sollten Sie keine Handlungen oder Bewegungen beschreiben, nur Gefühle, Meinungen und mentale Reaktionen. Schreiben Sie Ihre Gedanken nur als Text, verwenden Sie keine Klammern, Parenthesen oder andere Formatierung.',
  ...
  jp: {
    ...
    inner_thoughts_instruction: '%の内面の思考（ヴァレリアンの視点、一人称、生の思考のみ、フォーマットなし）。これは重要です：内面の思考では、行動や動きを描写せず、感情、意見、精神的反応のみを描写してください。思考はテキストとしてのみ書いてください。括弧、引用符、その他のフォーマットは使用しないでください。',
