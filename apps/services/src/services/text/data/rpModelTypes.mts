...
export const validateServicesSettings = (
  ...
  settings.rp_models?.forEach((model, index) => {
    if (model?.model_id_for_select) {
      const rpSelectModel = settings.rp_models.find(
        (m) => m.id === model.model_id_for_select
      );
      if (!rpSelectModel) {
        errors.push({
        ...
        model.permission === "free" &&
        rpSelectModel.permission !== "free"
      ) {
        errors.push({
          field: `rp_models[${index}].model_id_for_select`,
