 ...
 *
 */
export class OpenAIAphroditeTokenGenerator extends Guidance.TokenGenerator
  .AbstractTokenGenerator {
  private baseUrl: string;
  private apiKey: string;
  private model: string;
  private defaultConfig?: OpenAIAphroditeConfig;

  constructor(params: {
    baseURL: string;
    apiKey: string;
    model: string;
    defaultConfig?: OpenAIAphroditeConfig;
  }) {
    super();
    ...
    this.defaultConfig = {
      ...
      max_tokens: params.defaultConfig?.max_tokens || 100,
    };
  }
  ...
  override async generateToken(
    ...
    const config: AxiosRequestConfig<OpenAIAphroditeCompletionParams> = {
      method: "post",
    ...
    const result = await axios<{
      choices: {
        text: string;
        logprobs?: { top_logprobs: Record<string, number>[] };
      }[];
    }>(config);
    const logprobsResult = result.data.choices[0].logprobs?.top_logprobs || [];
  ...
  override async *generateString(
    ...
    const stream = await axios(config);

    const resolveObj: { resolveChunk: (value: string) => void } = {
      resolveChunk: () => {
        console.warn("invalid resolveChunk function");
      },
    };

    stream.data.on("data", (chunk: Buffer) => {
      try {
        const dataString = chunk.toString().slice(6);
        if (dataString.includes("[DONE]")) {
          resolveObj.resolveChunk("<DONE>");
          return;
        }
        const data = JSON.parse(dataString) as { choices: { text: string }[] };
        resolveObj.resolveChunk(data.choices[0].text);
      } catch (error) {
        resolveObj.resolveChunk("<DONE>");
      }
    });
    ...
    while (true) {
      const chunk = await new Promise<string>((resolve) => {
        resolveObj.resolveChunk = (chunk: string) => {
          if (chunk === "<DONE>") {
            resolve("");
          } else {
            result += chunk;
