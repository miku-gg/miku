...
export default function ActionEditModal() {
  ...
  const handleScrollToTop = useCallback(() => {
    if (containerRef.current) {
      if (
        action?.usageActions &&
        action?.usageActions?.length > prevActionsLength
      ) {
        scrollToTop();
      ...
      {action ? (
        <div className="ActionEdit">
          ...
          {action.usageActions && action.usageActions.length > 0 ? (
            <div
              className="ActionEdit__mutationList scrollbar"
              ref={containerRef}
            >
              {action.usageActions.map((act, index) => (
                <div
                  key={`${act.type}-${index + 1}`}
                  className="ActionEdit__mutationList__usageAction"
                >
                  <FaTrashAlt
